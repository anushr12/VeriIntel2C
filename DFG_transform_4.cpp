	...
	if(first_EqualTo_node){
		...
		while(first_n){
			...
			if(state_exists){
				...
				//EDITING HERE
				//in this case, it also must be checked if there are any comparators in the graph,since this has no state.
				//thus, every first_n is traced depthwise to check for comparator. if found, then a basic block is created for every comparator found.
	...
	//parallel_place means its created because there is a "input" type rhs_value to this place
		...
		while(tmp_p){
			if(tmp_p->state_place_input){
				...
				//states will be made when this is called for the first time
...
// Search for the arrays EDIT HERE
//Search_array_from_dfg(first_basic_block);
...
if(control_create && equalnodes_exist){
	...
	if(search_control){
		if(!search_control->bb_ptr){
			search_control->bb_ptr = new Basic_block;
			Initialize_nodes(NULL,NULL,NULL,NULL,search_control->bb_ptr,NULL);
			Insert_in_BasicBlock_list(NULL,search_control->bb_ptr);
		}
		Insert_path_in_BasicBlock(search_control,search_control->bb_ptr,pathnode,first_state,NULL,false);
	}
	 	...
	 	while(tmpvar){
	 		if(Search_var_from_statelist(tmpvar,tmpstate,false) && Search_state_from_controlnode_list(tmpstate,first_control_node,control_prev)){
	 			...
	 			if(tmpvar == control_out->signalnode->state_in_signal)
		 			...
		 			Insert_in_Controls_array(tmp_control,10,control_prev);
				...
				while(tmp_place && Search_var_in_InputList(tmp,tmp_place,NULL)){
					...
					if(Search_end_node_BasicBlock(var,first_basic_block,endnode,controlnode,true,block_ret,false,NULL)){
						...
						if(!loop_start && tmp_control){
							...
							else if(!tmp_control->result_control->control_dfg_node)  Insert_in_child_list(tmp_control->result_control,control_prev);
						}
						if(tmp_place->state_place_input){
							if(Search_var_from_statelist(tmp_place->state_place_input,tmpstate,false) && Search_state_from_controlnode_list(tmpstate,first_control_node,cntrl_empty)){
								...
								else if(!cntrl_empty->result_control->control_dfg_node)  Insert_in_child_list(cntrl_empty->result_control,control_prev);
							}
						}
					}
						...
						if(Search_var_from_statelist(tmpvar,tmpstate,false) && Search_state_from_controlnode_list(tmpstate,first_control_node,control_prev)){
							...
							if(tmpvar == tmp->signal_selector_out->state_in_signal){
								...
								if(!loop_start && tmp_control){
									...
									else if(tmp_control->result_control->control_dfg_node)  Insert_in_child_list(tmp_control->result_control,control_new);
								...
								else if(!control_prev->result_control->control_dfg_node)  Insert_in_child_list(control_prev->result_control,control_new);
...
bool checkstate=false , loop_del = true;
bool node_down=false;
bool no_state=false;
...
if(Search_back_petri_model(varsearch,NULL,true,true,first_state,i,false,loop_node,false,var_list,10,search_dfg,block_ret,false,endnode)){
	...
	//to check if the looop already exists in DFG CHANGED HERE NOW!! EDIT 1
	if(search_dfg && Search_node_in_DataLoop(varsearch,varsearch,endnode,tmp_exit))
		create_loop = false;
...
if(start){
	...
	if(loop_type){
		...
		if(!search_dfg && Check_exitpoint_of_loop(header_node,NULL,NULL,exit_node,controlnode_check,true,true))
			control_flow = true;
		else{
			control_flow = false;
		...
		do{
			...
			if(loop_list[i] && loop_list[i]->placenode){
				...
				if(tmp_place->state_place_input){
					...
					tmpeq = NULL;
					//check if output of tmp_place is already in the DFG graph 	 
		...
		if(check){
				...
				//here we are assuming that the comp node is just a part of the control loop
					...
					//	else Insert_in_control_list(control_ret->result_control,loop_block);
						...
						Insert_in_Controls_array(loop_block,10,control_ret);
...
/*
	if(searchflag && !start){
		if(!strcmp(tmp->name,var->name) || Check_node_from_array(arr_ptr,10,tmp)){
			flag = false;
			break;
		}
	}*/
	...
	if(!flag && tmp->equal_op_out){
		//check if the tmpeq is a part of the loop or not
		tmpeq = tmp->equal_op_out;
		while(tmpeq){
			//_node_in_array(arr_ptr,10,tmpeq->lhs_value,true);
	...
	if(!flag && tmp->operator_out){
		//Change_node_in_array(arr_ptr,10,tmp->operator_out->output_value,true);
	...
	if(!flag && tmp->transition_out){
		//Change_node_in_array(arr_ptr,10,tmp->transition_out->output_var,true);
...
if(!flag && !search_basic_block){
	...
	Initialize_array(dom_arrptr,100);
}

else if(flag && !search_basic_block){
	tmp_state = state;
	 if(Check_node_from_array(dom_arrptr,100,varstart))
	 	loop_start = false;	
}
...
while(loop_start && (tmp_state || first_input)){
	...
	while(tmpvar){
		...
		Change_node_in_array(dom_arrptr,100,tmpvar,true); //insert here
			...
			while(tmpeq){
				...
				if(!strcmp(searchvar->name,tmpeq->lhs_value->name)){
					...
					if(dominator_only){
						...
						search = true;
						//CHANGED HERE FIRST!!!
						if(Search_dominator_of_node(searchvar,tmpeq->lhs_value,node,first_state,tmp_state,true,part_of_loop,dominator_only,search_basic_block,block_ret,dom_arrptr,tmp_block))
							search = true;
					}
					else if(!dominator_only && part_of_loop){
						//Initialize_array_pointer(node);
...
// fn to check the output of nodestart
bool Check_node_out(DeclareData *nodestart, DeclareData *nodetocompare){

bool search = false;
Equal_to_op *tmpeq = NULL;

if(nodestart->equal_op_out){
	tmpeq = nodestart->equal_op_out;
	while(tmpeq){
		if(!strcmp(tmpeq->lhs_value->name , nodetocompare->name)){
			search = true;
			break;
		}	
		tmpeq = tmpeq->next_equal;
...
if(!search && nodestart->operator_out){
	if(!strcmp(nodestart->operator_out->output_value->name,nodetocompare->name))
		search = true;
}
...
Control_block *control_ret = NULL;
Basic_block *block_ret=NULL , *block_null = NULL , *block_ret_tmp = NULL, *block_temp = NULL;
DeclareData *endnode=NULL , *endnode_tmp = NULL;
DeclareData *firstnode = NULL , *endnode_out = NULL , *temp_node = NULL;
Control_block *controlnode=NULL , *controlnode_tmp = NULL;
Control_block *loopcontrol = NULL;
Operate_op *tmpop = NULL , *prevop = NULL;
Path_list *tmp_exit = NULL , *exit_points = NULL;
Path_list *nodecheck = NULL;
Path_list *loopempty=NULL;
Path_list *header = NULL;
Place_block *tmp_place=NULL;
Condition_block *tmp_cond=NULL;
bool node_down=false , path_done = false;
bool loopflag = true;
bool stateflag=false;
...
//do{
	...
	while(depth_tmp){
		...
		if(!node_down && depth_tmp->operator_out){
			...
			endnode = NULL; controlnode = NULL; endnode_out = NULL;block_ret_tmp = NULL;temp_node = NULL; block_temp = NULL;
			...
			if((!loop_nodesend|| !loopflag)  && (Search_end_node_BasicBlock(depth_tmp,first_basic_block,temp_node,controlnode,true,block_temp,false,NULL) && !Check_node_out(temp_node,depth_tmp->operator_out->output_value))){
				...
				if(temp_node && Search_node_in_DataLoop(temp_node,temp_node,temp_var,exit_points))
					break;
				else
					Insert_path_in_BasicBlock(control_st,datablock,depth_tmp->operator_out->output_value,first_state,NULL,loop_nodesend);
			}
		}
		if(!node_down && depth_tmp->equal_op_out){
			...
			while(eq_node){
				...
				endnode = NULL; controlnode = NULL; endnode_out = NULL;block_ret = NULL;block_ret_tmp = NULL;temp_node = NULL;block_temp =NULL;
				if((!loop_nodesend || !loopflag) && (Search_end_node_BasicBlock(depth_tmp,first_basic_block,temp_node,controlnode,true,block_temp,false,NULL) && !Check_node_out(temp_node,eq_node->lhs_value))){
		...
		if(!node_down && depth_tmp->place_ptr){
			...
			while(tmp_place && Search_var_in_InputList(depth_tmp,tmp_place,NULL)){
				...
				endnode = NULL; controlnode = NULL; endnode_out = NULL;block_ret_tmp = NULL; temp_node = NULL;block_temp = NULL;
				//APPLY THIS RULE TO OTHER NODES
				if((!loop_nodesend || !loopflag) && (Search_end_node_BasicBlock(depth_tmp,first_basic_block,temp_node,controlnode,true,block_temp,false,NULL) && !Check_node_out(temp_node,tmp_place->first_trans_node->output_var))){
							...
							if(strcmp(temp->name ,temp_var->name)){
								if(temp->unary_operator||Search_var_from_graph(temp,NULL,NULL,prev_comp,prev_signal,prevop,prev_var,first_state,false,true)){
									if(Search_control_nodes(NULL, controlnode ,node_empty, temp,NULL)){
										...
										Insert_in_BasicBlock_array(datablock,10,controlnode);
										stateflag=true;
										break;
									}	
					...
					if(stateflag){
						...
						if(depth_tmp->data_type == input || Search_end_node_BasicBlock(depth_tmp , first_basic_block ,endnode,controlnode,true, block_ret,false,NULL)){
							...
							if(loop_nodesend && Search_end_node_BasicBlock(tmp_place->first_trans_node->output_var,first_basic_block,endnode_out,controlnode,true,block_ret_tmp,false,NULL)){
								if(endnode->equal_op_out && !strcmp(endnode->equal_op_out->lhs_value->name,endnode_out->name))
									path_done = true;
								else{
							...
							//EDITING TO DO HERE! edit the calling in a way that if endnode_tmp exists, then
							else if(block_ret == control_st->bb_ptr)
								Create_subnodes_under_endnode(endnode ,depth_tmp ,control_st,block_ret ,tmp_place,block_null,tmpeq,NULL,tmpop,NULL,true, 'p');
							else
								Create_subnodes_under_endnode(endnode ,depth_tmp ,control_st,datablock ,tmp_place,block_ret,tmpeq,NULL,tmpop,NULL,true, 'p');
							//node_down = true;
							if(prev_control)
		...
		if(!node_down && depth_tmp->transition_out){
			...
			endnode = NULL; controlnode = NULL; endnode_out = NULL;block_ret_tmp = NULL; temp_node = NULL;block_temp = NULL;
			if((!loop_nodesend || !loopflag) && (Search_end_node_BasicBlock(depth_tmp,first_basic_block,temp_node,controlnode,true,block_temp,false,NULL) && !Check_node_out(temp_node,depth_tmp->transition_out->output_var))){
			
				if(Search_end_node_BasicBlock(depth_tmp , first_basic_block ,endnode,controlnode,true, block_ret,false,NULL)){
					if(datablock == block_ret)
					 Create_subnodes_under_endnode(endnode,depth_tmp,controlnode,block_ret ,NULL,block_null,tmpeq,depth_tmp->transition_out->data_node->assignment_graph,tmpop,NULL,true, 't');
					 else
		...
		if(!node_down && depth_tmp->comparator_out && !Search_end_node_BasicBlock(depth_tmp->comparator_out->value_out,first_basic_block,endnode,controlnode,true,block_ret,false,NULL)){
		...
		if(!node_down && depth_tmp->signal_selector_out && !Search_end_node_BasicBlock(depth_tmp->signal_selector_out->out_signal,first_basic_block,endnode,controlnode,true,block_ret,false,NULL)){
		...
		//here no need to check if the output of dfgnode is available. it already checks it inside
		if(!node_down && depth_tmp->dfgnode_out){
			...
			if(!loop_nodesend || !loopflag){
				//here the depth_tmp must be searched for the appropriate tmp_cond
				Get_cond_from_dfgnode(depth_tmp->dfgnode_out,depth_tmp,tmp_cond);
...
//fn to search the appropriate condition_block from dfg_switchcase whose input is var_compare
void Get_cond_from_dfgnode(DFG_block *dfg_switchcase,DeclareData *var_compare,Condition_block*& cond_ret){

Condition_block *tmp = NULL;

tmp = dfg_switchcase->condition_op;
while(tmp){
	if(!strcmp(var_compare->name, tmp->assign_expr->rhs_value->name)){
		cond_ret = tmp;
		break;
	}
	tmp = tmp->next_cond;
...
if(Search_end_node_BasicBlock(condvar, first_basic_block ,end_node,control_node,true, block_ret,false,NULL)){
		...
		Insert_in_Controls_array(control_create,10,control_prev);
		control_prev = control_node;
...
//this is the part for the other cases of the switch-case node, attaching the equal nodes in parallel
 if( condition_found && (Search_end_node_BasicBlock(depth_node, first_basic_block ,end_node,control_node,true, block_ret,false,NULL) || Check_parent_nodes(depth_node,first_state,NULL,control_node,end_node,block_ret))){
	...
	//control_node is not obtained from the Search_endnode() function.
		...
		else if(control_node && !control_node->result_control->control_dfg_node) Insert_in_child_list(control_node->result_control,control_create);
}
...
//insert it as a block input first
	...
	else if(!control_node->result_control->control_dfg_node)  Insert_in_child_list(control_node->result_control,control_create);
...
if(tmp_comp){
	...
	if(flag){
		...
		if(tmp_comp->state_condition && control_st){
			if(control_st->state_node && !strcmp(tmp_comp->state_condition->state_name->name , control_st->state_node->state_name->name)){
				...
				else if(!control_st->result_control->control_dfg_node) Insert_in_child_list(control_st->result_control , tmp_control);
			...
			if(Search_control_nodes(NULL , control_ret,node_ret,tmp_comp->condition_value,NULL)){
				...
				else if(!control_ret->result_control->control_dfg_node)  Insert_in_child_list(control_ret->result_control,tmp_control);
	...
	if(control_attach){
		...
		else if(!control_attach->result_control->control_dfg_node)  Insert_in_child_list(control_attach->result_control,tmp_control);
	}
}
else if(tmp_signal){
	...
	if(flag){
		...
		if(endnode){
			...
			if(!endnode->signal_selector_out)
				...
				insert_in_operator_list(endnode , NULL,tmp_control->signal_select_node,false,NULL,false);
		}
	...
	//RULE here is: if endnode is a lhs_compare of a control_compare node, then it cannot be a  control_out of control_attach. its control_out should only point to the resultant value of the control_compare node
...
//	assign_value(tmp_control->control_compare_node->value_compare , tmpcond->equal_cond->rhs_value);
	...
	//tmp_control->control_in = control_attach;
		Insert_in_Controls_array(tmp_control,10,control_st);
	
	if(!endnode->comparator_out)
		endnode->comparator_out = tmp_control->control_compare_node;
	else Insert_in_compare_list(endnode->comparator_out,tmp_control->control_compare_node);
	
	if(control_attach){
		//if(!control_attach->control_out) control_attach->control_out = endnode;
		//tmp_control->control_in = control_attach;
		Insert_in_Controls_array(tmp_control,10,control_attach);
		if(!control_attach->result_control) control_attach->result_control = tmp_control;
		else if(!control_attach->result_control->control_dfg_node)  Insert_in_child_list(control_attach->result_control,tmp_control);
	}
...
while(block){
	...
	while(tmp){
		...
		if(!node_down && Search_node_in_DataLoop(nodetocompare,tmp,node_ret,exit_nodes)){
		//there are two cases: when the nodetocompare node is found ,and when its not found, exit_nodes are rturned. now, search nodetocompare from the path of every exit_nodes
			if(node_ret){
				...
				//the exit_nodes are connected via variable nodes to the loop_path. here, the path should be traced from output of exit node
					...
					if(Search_end_node_BasicBlock(nodetocompare,block,node_ret,cntrl_ret,true,node_block_ret,true,tmpvar)){
							node_down = true;
							break;
					}
		...
		if(!node_down && tmp->operator_out){
			...
			while(tmpop){
				...
				if(!Search_end_node_BasicBlock(nodetocompare,block,node_ret,cntrl_ret,search_block,node_block_ret,true,tmpop->output_value)){
					node_down = false;
					tmpop = tmpop->next_oper;
				}
				else tmpop=NULL;
		...
		if(!node_down && tmp->equal_op_out){
			...
			while(tmpeq){
				...
				node_down=true;
				if(!Search_end_node_BasicBlock(nodetocompare,block,node_ret,cntrl_ret,search_block,node_block_ret,true,tmpeq->lhs_value)){
					node_down = false;
					tmpeq = tmpeq->next_equal;
				}
				else tmpeq=NULL;
...
bool Search_node_in_DataLoop(DeclareData *nodetocompare,DeclareData *startfromvar,DeclareData*& node_found,Path_list*& exit_point){
...
//fn to insert the control_in pointer in the list of control entry pointers of block_attach
void Insert_in_Controls_array(Control_block *block_attach,int size,Control_block *control_in){

int i =0;
for(i=0; i<size;i++){
	if(block_attach->control_in_pointers[i])
		continue;
	else
		break;
}
block_attach->control_in_pointers[i++] = control_in;
